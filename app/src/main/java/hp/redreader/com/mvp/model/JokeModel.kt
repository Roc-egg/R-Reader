...
class JokeModel
@Inject
constructor(repositoryManager: IRepositoryManager) : BaseModel(repositoryManager), JokeContract.Model {
    override fun getQsbkList(mPage: Int, evictCache: Boolean): Observable<QsbkListBean> {
        return Observable.just(mRepositoryManager
                .obtainRetrofitService(DouBanService::class.java)
                .getQsbkList(mPage))
                .flatMap<QsbkListBean> { listObservable ->
                    mRepositoryManager.obtainCacheService(CommonCache::class.java)
                            .getQsbkList(listObservable,
                                    DynamicKey(mPage)
                                    , EvictDynamicKey(evictCache))
//                            .map { listReply -> listReply.data }
                            .map { reply: Reply<QsbkListBean> ->
                                reply.data.apply {
                                    when (reply.source) {
                                        Source.CLOUD -> {
                                            Timber.i("數據來自網諾")
