...
class AndroidModel
@Inject
constructor(repositoryManager: IRepositoryManager) : BaseModel(repositoryManager), AndroidContract.Model {
    override fun getGankIoData(mType: String, mPage: Int, preEndIndex: Int, evictCache: Boolean): Observable<GankIoDataBean> {
        return Observable.just(mRepositoryManager
                .obtainRetrofitService(GankService::class.java)
                .getGankIoData(mType, mPage, preEndIndex))
                .flatMap<GankIoDataBean> { listObservable ->
                    mRepositoryManager.obtainCacheService(CommonCache::class.java)
                            .getGankIoData(listObservable,
                                    DynamicKey(mType + "_" + mPage)//同一个接口缓存的区分
